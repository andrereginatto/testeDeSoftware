...
if(isset($_POST['save']) && isset($_GET['id'])){
    if(strlen($_POST['categoria']) <= 20 && strlen($_POST['categoria']) > 1){
        $result = $c->update_categoria($_GET['id'], $_POST['categoria']);
        if(isset($erro)){
            unset($erro);
        }
        $linha = $c ->select_by_id_cat($_GET['id']);
    }else{
        $erro=true;
        $msg_erro = '- Por favor, a categoria deve haver entre 2 e 20 caracteres.<br>';
    }
}else if(isset($_POST['delete'])){
    $result = $c->delete_categoria($_GET['id']);
    header('Location: ./');
    ...
    $categorias = $c->select_by_id_user($_SESSION['id']);
}
    ...
    <body id="body">
        ...
        <div id="main">
            <div class="container" style="box-shadow: none; background-color: transparent; margin-top: 0px;">
                ...
                <div class="container">
                    <div class="titulo_div"><strong style="margin-left: 4px; margin-right: 4px;">Editar Categoria</strong></div>
                    ...
                    <form method="get" onsubmit="return validaFormCadastro();" >
                            ...
                            <label for="categoria" style="margin-top: 3px;">Selecione a Categoria:</label>
                            ...
                            <?php
                               foreach ($categorias as $linhas){
                                   echo '<option value="'.$linhas['ID'].'">'.$linhas['CATEGORIA'] .' </option>';
                               }
                            ?>
                        ...
                        <div class="direita" style="float: right;">
                           ...
                           <button type="submit" style="float: right; margin-bottom: 7px;margin-top: 5px;" class="btn btn-small btn-info">
                    ...
                    ?>
                        ...
                        <form method="post" onsubmit="return validaFormCadastro();">
                            ...
                            <input type="text" class="form-control" name="categoria" style="margin-top: -7px; margin-bottom: 3px; width: 100%;"
                               maxlength="20" id="categoria" placeholder="Categoria" 
                                   <?php 
                                        if(isset($erro)){
                                            echo ' autofocus';
                                            if($erro){ 
