...
if (isset($_POST['save']) && !isset($_GET['id'])) {
    ...
    if (!$erro) {
        if($diadehoje < $_POST['data']){
            ...
            $result = $automa->insertIntoAutomatico($_POST['conta'], $_POST['valor'], $_POST['data'], 'G', $parcela['PARCELA'], $_POST['obs'], $_POST['categoria']);
            unset($erro);
            header('Location: ./');
        }else{
            $result = $p->insert_gasto($_POST['valor'], $_POST['obs'], $_POST['conta'], $_POST['data'], $_POST['categoria']);
            unset($erro);
            header('Location: ./');
        }
    }
    ...
    if (!$erro) {
        ...
        /*$linha = $p->select_gasto_by_id($_GET['id']);
        $t->update_to_null($linha['PARCELA']);*/
        if($diadehoje < $_POST['data']){
            require_once './Classes/DAO_AUTOMATICOS.php';
            $automa = new DAO_AUTOMATICOS();
            $p->delete_gasto_by_id($_GET['id']);
            $parcela= $automa->returnProximaParcela();
            $result = $automa->insertIntoAutomatico($_POST['conta'], $_POST['valor'], $_POST['data'], 'G', $parcela['PARCELA'], $_POST['obs'], $_POST['categoria']);
            unset($erro);
            header('Location: ./');
        }else{
            $result = $p->update_gasto($_POST['valor'], $_POST['obs'], $_POST['conta'], $_POST['data'], $_GET['id'], $_POST['categoria']);
            unset($erro);
            header('Location: ./');
        }
    ...
    if(!isset($linha['VALOR']) && !isset($erro)){
        $erro=true;
        $msg_erro = 'Gasto não encontrado.';
    }
} else if (isset($_POST['delete'])) {
    $result = $p->delete_gasto_by_id($_POST['delete']);
    header('Location: ./');
} else if (!isset($_POST['save']) && isset($_GET['id'])) {
    $linha = $p->select_gasto_by_id($_GET['id']);
...
<?php include_once 'html_usuario.php'; ?>
                <div class="container">
                    <div class="titulo_div"><strong style="margin-left: 4px; margin-right: 4px;"><?php if(isset($_GET['id'])){echo 'Editar ';}else{echo"Adicionar ";}; ?>Gasto</strong></div>
                    ...
                    ?>
                        ...
                        <form method="post" onsubmit="return validaFormCadastro();">
                            <div class="esquerda" style="float: left;">
                                ...
                                <div class="input-group" style="margin-top: -7px; margin-bottom: 3px;">
                                    <span class="input-group-addon" id="basic-addon1">
                                        <span class="glyphicon glyphicon-minus"> </span></span>
                                    <input type="number" class="form-control" name="valor" step="0.01" required="" max="999999999999.99" min="0.01"
                                           id="valor" placeholder="50,99"  aria-describedby="basic-addon1"
    <?php
    if (isset($erro)) {
        if ($input_erro == 'valor') {
            echo ' autofocus';
    ...
    ?>>
                            ...
                            <div class="direita" style="float: right;">
                                ...
                                <input type="date" class="form-control" name="data" style="margin-top: -7px; margin-bottom: 3px;"
                                       id="data" required=""
                                       <?php
                                       if (isset($erro)) {
                                           if ($input_erro == 'data') {
                        ...
                        if (isset($result)) {
                            if ($result == 1) {
                                echo '<center><strong id="return-banco" style="color: green; font-size:17px; font-family: Cabin;">Gasto alterado com Sucesso!</strong></center>';
                            }
                        }
                        ?>
                        <strong style="color: red; font-family: Cabin;" id="mensagem_erro"></strong>
                        <strong style="color: red; font-family: Cabin;" id="erro_php">
                            <?php
                            if (isset($erro) && $erro == true) {
                                echo $msg_erro;
                            }
